...
{% macro duckdb__synth_column_date_sequence_base(start_date, step) %}
    {% if start_date|length ==0 %}
    (CURRENT_DATE + interval '{{step}} day' * (__row_number-1))::date
    {% else %}
    (date '{{start_date}}' + interval '{{step}} day' * (__row_number-1))::date
    {% endif %}
{% endmacro %}
