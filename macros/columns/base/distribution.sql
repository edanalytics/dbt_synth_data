...
-#}
{% macro column_distribution(name, class, type, min=0, max=1, mean=0, stddev=1, lambda=1.0, precision=-1, n=10, p=0.5, probabilities=None, wrap="") %}
    {% if class=='discrete' %}
    {{ dbt_synth.distribution_discrete(type, min=min, max=max, mean=mean, stddev=stddev, n=n, p=p, probabilities=probabilities, wrap=wrap) }}
