{% macro synth_column_integer(name, distribution=None, min=None, max=None) -%}
    {% if distribution is none and min is none and max is none %}
        {{ exceptions.raise_compiler_error("intever column `" + name + "` must specify either `min` and `max` (for a uniform distribution) or `distribution` (your own distribution of integers)") }}
