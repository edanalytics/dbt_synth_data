...
{% macro sqlite__synth_column_string_base(name, min_length, max_length) %}
    {% set cte %}
        {{name}}__cte(n, v, chars) as (
            select {{ dbt_synth_data.synth_distribution_discretize_floor(
                    distribution=dbt_synth_data.synth_distribution_continuous_uniform(min=min_length, max=max_length+1)
                ) }}, '', '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
            UNION ALL
            SELECT n-1, v||SUBSTR(chars, {{ dbt_synth_data.synth_distribution_discretize_floor(
                distribution=dbt_synth_data.synth_distribution_continuous_uniform(min=0, max='LENGTH(chars)')
            ) }} + 1, 1), chars
            FROM {{name}}__cte
            WHERE n > 0
        )
