...
{% macro postgres__synth_column_foreign_key(name, table, column) %}
    {{ synth_add_update_hook(postgres__synth_column_foreign_key_update(name, table, column)) or "" }}
    {{ synth_add_cleanup_hook(postgres__synth_column_foreign_key_cleanup(name)) or "" }}
    
    RANDOM() AS {{name}}_rand,
    ''::varchar as {{name}}
{% endmacro %}

{% macro postgres__synth_column_foreign_key_update(name, table, column) %}
...
{% macro postgres__synth_column_foreign_key_cleanup(name) %}
...
{% macro snowflake__synth_column_foreign_key(name, table, column) %}
    {{ synth_add_update_hook(snowflake__synth_column_foreign_key_update(name, table, column)) or "" }}
    {{ synth_add_cleanup_hook(snowflake__synth_column_foreign_key_cleanup(name)) or "" }}
    
    UNIFORM(0::float, 1::float, RANDOM( {{synth_get_randseed()}} )) AS {{name}}_rand,
    ''::varchar as {{name}}
{% endmacro%}

{% macro snowflake__synth_column_foreign_key_update(name, table, column) %}
...
{% macro snowflake__synth_column_foreign_key_cleanup(name) %}
alter table {{ this }} drop column {{name}}_rand
{% endmacro %}