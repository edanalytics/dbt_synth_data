{% macro synth_column_lookup(name, model_name, value_col, from_col, to_col) -%}
    ...
    {% endset %}
    {% set join_clause %}
        left join {{ref(model_name)}} {{name}}__lookup on ___PREVIOUS_CTE___.{{value_col}}={{name}}__lookup.{{from_col}}
    {% endset %}
