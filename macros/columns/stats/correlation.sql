...
{% macro postgres__synth_column_correlation(data, column) %}
    {{ synth_add_update_hook(postgres__synth_column_correlation_update(data, column)) or "" }}
    {{ synth_add_cleanup_hook(postgres__synth_column_correlation_cleanup(column)) or "" }}
...
{% macro postgres__synth_column_correlation_update(data, column) %}
...
{% macro postgres__synth_column_correlation_cleanup(column) %}
...
{% macro snowflake__synth_column_correlation(data, column) %}
    {{ synth_add_update_hook(snowflake__synth_column_correlation_update(data, column)) or "" }}
    {{ synth_add_cleanup_hook(snowflake__synth_column_correlation_cleanup(column)) or "" }}
...
{% macro snowflake__synth_column_correlation_update(data, column) %}
    {%- set hypercube_dimension = data.columns|length -%}
    {%- set hypercube_shape = [] -%}
    {%- set ns = namespace(column_index=0) -%}
    {%- set ns2 = namespace(counter=0) -%}
...
{% macro snowflake__synth_column_correlation_cleanup(column) %}
alter table {{ this }} drop column {{column}}_rand
{% endmacro %}
