{% macro synth_phone_number(name) -%}
    {{ return(adapter.dispatch('synth_phone_number')(name)) }}
{%- endmacro %}

{% macro default__synth_phone_number(name) -%}
    {# NOT YET IMPLEMENTED #}
{%- endmacro %}

{% macro postgres__synth_phone_number(name) %}
...
{% macro snowflake__synth_phone_number(name) %}
