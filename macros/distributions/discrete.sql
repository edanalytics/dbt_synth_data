...
{% macro distribution_discrete(type, min=0, max=1, mean=0, stddev=1, lambda=1.0, precision=-1, n=10, p=0.5, probabilities=None, wrap="") -%}
    {%- if type=='uniform' -%}
    ...
    {{ dbt_synth.distribution_discrete_exponential(lambda=lambda) }}
    {%- elif type=='bernoulli' -%}
