{% macro synth_distribution(name, class, type, min=0, max=1, mean=0, stddev=1, lambda=1.0, n=10, p=0.5, probabilities=None, values=None, weights=None) %}
    {% if class=='continuous' %}
...
{% macro synth_distribution_continuous(type, min=0, max=1, mean=0, stddev=1, lambda=1.0) -%}
    {%- if type=='uniform' -%}
