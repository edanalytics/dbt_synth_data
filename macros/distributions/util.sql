{% macro synth_distribution(name, class, type, min=0, max=1, mean=0, stddev=1, lambda=1.0, mu=0.0, b=1.0, x0=0.0, gamma=1.0, n=10, p=0.5, probabilities=None, values=None, weights=None) %}
    {% if class=='continuous' %}
    {{ dbt_synth_data.synth_distribution_continuous(type, min=min, max=max, mean=mean, stddev=stddev, lambda=lambda, mu=mu, b=b, x0=x0, gamma=gamma) }}
    {% elif class=='discrete' %}
    {{ dbt_synth_data.synth_distribution_discrete(type, n=n, p=p, probabilities=probabilities, values=values, weights=weights) }}
    {% else %}
...
{% macro synth_distribution_continuous(type, min=0, max=1, mean=0, stddev=1, lambda=1.0, mu=0.0, b=1.0, x0=0.0, gamma=1.0) -%}
    {%- if type=='uniform' -%}
    {{ dbt_synth_data.synth_distribution_continuous_uniform(min=min, max=max) }}
    {%- elif type=='normal' -%}
    {{ dbt_synth_data.synth_distribution_continuous_normal(mean=mean, stddev=stddev) }}
    {%- elif type=='exponential' -%}
    {{ dbt_synth_data.synth_distribution_continuous_exponential(lambda=lambda) }}
    {%- elif type=='laplace' -%}
    {{ dbt_synth_data.synth_distribution_continuous_laplace(mu=mu, b=b) }}
    {%- elif type=='cauchy' -%}
    {{ dbt_synth_data.synth_distribution_continuous_cauchy(x0=x0, gamma=gamma) }}
    {%- else -%}
...
{% macro synth_distribution_discrete(type, n=10, p=0.5, probabilities=None, values=None, weights=None) -%}
    {%- if type=='bernoulli' -%}
    {{ dbt_synth_data.synth_distribution_discrete_bernoulli(p=p) }}
    {%- elif type=='binomial' -%}
    {{ dbt_synth_data.synth_distribution_discrete_binomial(n=n, p=p) }}
    {%- elif type=='probabilities' -%}
    {{ dbt_synth_data.synth_distribution_discrete_probabilities(probabilities=probabilities) }}
    {%- elif type=='weights' -%}
    {{ dbt_synth_data.synth_distribution_discrete_weights(values=values, weights=weights) }}
    {%- else -%}
