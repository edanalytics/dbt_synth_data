{% macro synth_select(name, value_col, lookup_table, distribution="uniform", weight_col="", filter="", funcs=[]) -%}
    
    {% if distribution=='uniform' %}
        {{ return(adapter.dispatch('synth_select_uniform')(name, value_col, lookup_table, filter, funcs)) }}
    
    {% elif distribution=='weighted' %}
        {% if weight_col=='' %}
            {{ exceptions.raise_compiler_error("`weight_col` is required when `distribution` for select column `" ~ name ~ "` is `weighted`.") }}
        {% endif %}
        {{ return(adapter.dispatch('synth_select_weighted')(name, value_col, lookup_table, weight_col, filter, funcs)) }}
...
{% macro default__synth_select_uniform(name, value_col, lookup_table, filter, funcs) -%}
    {# NOT YET IMPLEMENTED #}
{%- endmacro %}

{% macro default__synth_select_weighted(name, value_col, lookup_table, weight_col, filter, funcs) -%}
    {# NOT YET IMPLEMENTED #}
{%- endmacro %}



{% macro postgres__synth_select_uniform(name, value_col, lookup_table, filter, funcs) %}
    {{ synth_add_update_hook(postgres__synth_select_uniform_update(name, value_col, lookup_table, filter, funcs)) or "" }}
...
{% macro postgres__synth_select_weighted(name, value_col, lookup_table, weight_col, filter, funcs) %}
    {{ synth_add_update_hook(postgres__synth_select_weighted_update(name, value_col, lookup_table, weight_col, filter, funcs)) or "" }}
    {{ synth_add_cleanup_hook(postgres__synth_select_cleanup(name)) or "" }}
...
{% macro postgres__synth_select_cleanup(name) %}
alter table {{ this }} drop column {{name}}_rand
{% endmacro %}



{% macro snowflake__synth_select_uniform(name, value_col, lookup_table, filter, funcs) %}
...
{% macro snowflake__synth_select_weighted(name, value_col, lookup_table, weight_col, filter, funcs) %}
...
{% macro snowflake__synth_select_cleanup(name) %}
alter table {{ this }} drop column {{name}}_rand
{% endmacro %}
